...
export class AuthService {
    ...
    login(correo: string, password: string) {
        ...
        //CONSULTAR LOCALSTORAGE CON LA KEY DE ABAJO QUE SE USA PARA GUARDAR EL RES DENTRO DEL IF,
        //SI EXISTE LA KEY Y HAY UN USUARIO REDIRIGIR DE UNA AL HOME
        //EN EL CASO CONTRARIO HACER TODO LO QUE VIENE DEBAJO


        this.db.readUser(correo, password, true).then((res) => {
            if(res !== null) {
                //INGRESAR EN LOCALSTORAGE UNA KEY DE USER CON EL RES EN JSON.STRINGIFY
                this.storage.setItem('USER_DATA', JSON.stringify(res));
                this.router.navigate(['/home']);
                return;
            }
            console.log('Credenciales incorrectas (o no existe o estan malas)');
        });
    }
    ...
    async login(correo: string, password: string) {
        ...
        try {
            ...
            console.log(`AuthenticationService.login. Obteniendo datos del usuario`);
